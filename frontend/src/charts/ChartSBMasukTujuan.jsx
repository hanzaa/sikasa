...
const ChartSBMasukTujuan = () => {
   ...
   const {
      data: dataSBMasuk,
      isError: isErrorSBMasuk,
      isFetching: isFetchingSBMasuk,
      isLoading: isLoadingSBMasuk,
      refetch: refetchSBMasuk,
   } = useQuery({
      queryKey: [
         "dataSBMasuk-data",
   ...
   const generateDataForChart = (data) => {
      ...
      data.forEach((item) => {
         const kepada = item["kepada"];
         if (kepadaCount[kepada]) {
            kepadaCount[kepada]++;
         } else {
            kepadaCount[kepada] = 1;
         }
      });

      const kepadaLabels = Object.keys(kepadaCount);
      const suratCount = Object.values(kepadaCount);
   ...
   const chartData = generateDataForChart(dataSBMasuk?.data ?? []);
...
export default ChartSBMasukTujuan;
